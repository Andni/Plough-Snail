...
bool ColorShader::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	result = InitializeShader(device, hwnd, L"../Plough-Snail/ColorVS.hlsl", L"../Plough-Snail/ColorPS.hlsl");
	if (!result)
	{
		return false;
	}
...
bool ColorShader::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
	...
	CompileVertexShader(vsFilename, &vertexShaderBuffer, &errorMessage, hwnd);
	CompilePixelShader(psFilename, &pixelShaderBuffer, &errorMessage, hwnd);
	
	CreateVertexShader(vertexShaderBuffer->GetBufferPointer(), vertexShaderBuffer->GetBufferSize(), NULL, &m_vertexShader, device, hwnd);
	CreatePixelShader(pixelShaderBuffer->GetBufferPointer(), pixelShaderBuffer->GetBufferSize(), NULL, &m_ixelShader, device, hwnd);
...
void ColorShader::CompileVertexShader(WCHAR* vsFilename, ID3D10Blob** vertexShaderBuffer, ID3D10Blob** errorMessage, HWND hwnd)
{
	...
	if (FAILED(result))
	{
		throw ShaderException(*errorMessage, vsFilename);
	}
}


void ColorShader::CompilePixelShader(WCHAR* psFilename, ID3D10Blob** pixelShaderBuffer, ID3D10Blob** errorMessage, HWND hwnd)
{
	...
	if (FAILED(result))
	{
		throw ShaderException(*errorMessage, psFilename);
	}
}
...
HRESULT ColorShader::CreateInputLayout(ID3D11Device* device, const void * vertexShaderBuffer, int vertexShaderBufferSize)
{
	...
	HRESULT result = device->CreateInputLayout(polygonLayout, numElements, vertexShaderBuffer,
		vertexShaderBufferSize, &m_layout);
	if (FAILED(result))
	{
		throw ShaderException("herp");
	}
}

void ColorShader::CreateVertexShader(__in  const void *pShaderBytecode,
	...
	HWND hwnd)
{
	HRESULT result = device->CreateVertexShader(pShaderBytecode, BytecodeLength, NULL, ppVertexShader);
	if (FAILED(result))
	{
		throw ShaderException("Creating vertex shader failed");
	}
}

void ColorShader::CreatePixelShader(__in  const void *pShaderBytecode,
	__in  SIZE_T BytecodeLength,
	__in_opt  ID3D11ClassLinkage *pClassLinkage,
	...
	HWND hwnd)
{
	HRESULT result = device->CreatePixelShader(pShaderBytecode, BytecodeLength, NULL, ppPixelShader);
	if (FAILED(result))
	{
		throw ShaderException("Creating pixel shader failed");
	}
}

void ColorShader::HandleShaderException(ShaderException e, HWND hwnd)
{
	...
	if (errorMessage)
	{
		...
		OutputShaderErrorMessage(errorMessage, hwnd, e.GetFilename());
	}
	// If there was  nothing in the error message then it simply could not find the file itself.
	else
	{
		MessageBox(hwnd, e.GetFilename(), L"Missing Shader File", MB_OK);
	}
}